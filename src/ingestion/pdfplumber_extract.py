...
def extract_images_with_pymupdf(
    ...
    """
    Extract embedded images from a PDF using PyMuPDF (fitz) and save them to output_dir.
    Returns a list of saved image paths.
    """
    try:
        import fitz
        ...
        for page_index in range(num_pages):
            ...
            for img_idx, img in enumerate(images):
                ...
                try:
                    ...
                    logging.warning("Failed to extract image on page %s (%s): %s", page_index + 1, xref, exc)
                    continue
                ...
                if not img_bytes:
                    continue
                filename = f"{Path(pdf_path).stem}_page{page_index + 1}_img{img_idx + 1}.{ext}"
                out_path = out_dir / filename
                try:
                    with open(out_path, "wb") as f:
                        f.write(img_bytes)
                    saved.append({"path": str(out_path), "page": page_index + 1})
                except Exception as exc:
                    logging.warning("Failed to save image %s: %s", out_path, exc)
